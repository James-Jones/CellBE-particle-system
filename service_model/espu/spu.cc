...
volatile SPUArgsExp control_block __attribute__((aligned(128)));
...
int main (unsigned long long speid, unsigned long long eaAddr, unsigned long long env __attribute__ ((unused)) )
{
	...
	spu_mfcdma64(&control_block, mfc_ea2h(eaAddr), mfc_ea2l(eaAddr), sizeof(SPUArgsExp), tag_id, MFC_GET_CMD);
	
	spu_writech(MFC_WrTagMask, tag_mask);
	spu_mfcstat(2);

  atomic_set(control_block.elast_index_a, 0);//no explosions
  ...
  explosions->set_target_size( control_block.array_length );
