...
volatile SPUArgs control_block __attribute__((aligned(128)));
...
int main (unsigned long long speid, unsigned long long eaAddr, unsigned long long env __attribute__ ((unused)) )
{
	...
	spu_mfcdma64(&control_block, mfc_ea2h(eaAddr), mfc_ea2l(eaAddr), sizeof(SPUArgs), tag_id, MFC_GET_CMD);
	
	spu_writech(MFC_WrTagMask, tag_mask);
	spu_mfcstat(2);

  atomic_set(control_block.elast_index_a, 0);
  ...
  rain->set_target_size(control_block.rain_array_length);
  rain->set_target(eal, eah);
  rain->initialize( &control_block );
  
  rain->validate_buffer_size();

  a.ull = control_block.exp_1_ea.ull;
  eah = mfc_ea2h(a.ui[0]);
  eal = mfc_ea2l(a.ui[0]);

  explosions = new ExplosionManager;
  explosions->set_target_size( control_block.exp_array_length );
  explosions->set_target(eal, eah);
  explosions->initialize();
  
  explosions->validate_buffer_size();
  ...
  if( response != SPU_GO )
  {
    ...
    if(val.us[0] == EXPLOSION)
    {
      explosions->add_task(val.us[1]);
    }
  ...
  rain->process_tasks();
  explosions->process_tasks();
  atomic_set(control_block.rlast_index_a, rain->get_updates());
  atomic_set(control_block.elast_index_a, explosions->get_updates());
  ...
  delta_time = timer.get_seconds();
    ...
    if( atomic_read(control_block.switch_buffer_a) == 1)
    {
      ...
      if (current_buffer == 0)
      {
        ...
        a.ull = control_block.exp_2_ea.ull;
        eah = mfc_ea2h(a.ui[0]);
        eal = mfc_ea2l(a.ui[0]);
        explosions->set_target(eal, eah);
      }
